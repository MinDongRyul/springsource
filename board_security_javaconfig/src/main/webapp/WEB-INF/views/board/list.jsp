          ...
          <div class="col-md-12">
            ...
            <div class="col-md-2 col-md-offset-2">
              ...
              <select name="" id="amount" class="form-control">
                <!-- out를 통해 현재 value가 10이라면 selected 실행 -->
...
<!-- /.row -->
<%-- 페이지 링크를 처리할 폼(폼처리시 pageNum과 amount를 보내줌) --%>
<form action="/board/list" id="actionForm">
	<!-- pageNum, amount, type, keyword 값을 부를 때 - 
	     1) pageDto(pageDto.cri.pageNum)
	     2) cri(criteria.pageNum(ModelAttribute가 사용 안된 경우), cri.pageNum(사용한 경우))
	-->
	<input type="hidden" name="pageNum" value="${cri.pageNum}" />
	<input type="hidden" name="amount" value="${cri.amount}" />
...
<script>
	...
	//컨트롤러에서 register성공시 result에 bno값을 넘겨 받아서 checkModal 실행
	let result = '${result}';
</script>
